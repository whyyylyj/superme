...
const TouchInput = {
    ...
    init: function () {
        ...
        // Prevent default touch behavior on the whole control area
        touchControls.addEventListener('touchstart', e => {
            ...
            // Check if we are on a control element to allow vibration
            if (e.target.closest('.action-btn, .pause-btn, .virtual-joystick')) {
                // No special action needed here, handled by individual listeners
            }
            e.preventDefault();
        }, { passive: false });
    },
